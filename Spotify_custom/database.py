...
class SpotifyDB():
    ...
    def initialize(self):
        if not os.path.exists("Spotify_custom/spotify.db"):
            ...
            self.exec("CREATE TABLE playlist (playlist_id STRING PRIMARY KEY, name STRING, owner STRING)")
            self.exec("CREATE TABLE song (song_id STRING PRIMARY KEY UNIQUE, name STRING, artists STRING, duration_ms INTEGER, popularity INTEGER)")
            self.exec("CREATE TABLE playlist_songs (playlist STRING REFERENCES playlist (playlist_id), song STRING REFERENCES song (song_id))")
            return
        self.database=sqlite3.connect("Spotify_custom/spotify.db",check_same_thread=False)
        self.db=self.database.cursor()
...
#-------------------------------------GetData----------------------------------------------
    def song(self,id):
        if not self.has_song(id):
            return None
        return self.exec("select * from song where song_id='"+id+"'",fetch=1)[0]
...
#-------------------------------------AddData-------------------------------------------------
    ...
    def add_playlist(self,id,name,owner):
        if not self.playlist_exists(id):
            self.exec("insert into playlist(playlist_id,name,owner) values(?, ?, ?)",[(id,name,owner)])
    ...
    def playlist_add_song(self,playlist,song):   
        ...
        for s in song:
            if not self.playlist_has_song(playlist,s[0]):
                self.exec("insert into playlist_songs (playlist,song) values(?,?)",[(playlist,s[0])])


    def playlist_delete_song(self,playlist,song):
        if self.playlist_has_song(playlist,song):
            self.exec("delete from playlist_songs where playlist='"+playlist+"' and song='"+song+"'")
#-------------------------------------UpdateData----------------------------------------------
    def updatePlaylist(self,id):
        ...
        self.exec("insert into playlist_songs(playlist,song) values(?, ?)",new_playlist_songs)
        self.add_song(new_songs)

#------------------------------------------------------------------------------------
    def clear_all_playlist_songs(self,id):
        self.exec("delete from playlist_songs where playlist='"+id+"'")

    def delete_old_playlists(self,playlists):
...
#-------------------------------------Utils--------------------------------------------------
    def playlist_json(self,id):
        ...
        while offs<=final["total"]:
            tmp=self.session.playlist_tracks(id,limit=100,offset=offs,market=None)
            tracks+=tmp["items"]
            offs+=100
        for song in tracks:
    ...
    def has_song(self,id):
        if not self.database_has(table="song",param1="song_id",value1=id):
            return False
        return True
    
    def database_has(self,table,param1,value1,param2=None,value2=None):
    ...
    def exec(self,command,params=None,fetch=None):
        if params==None:
